            ...
            quality.append(
                {
                    ...
                    "entity_type": "document",
                    "entity_id": sha256,
                    "check_name": "zip_signature",
                    "message": f"ZIP signature missing: {info['raw_path']}",
                }
            )
...
def build_parquet_metadata(
    ...
    for event in events:
        company_id = event.get("company_id") or source_company.get(event["source_id"])
        if not company_id and event.get("category") != "regulatory_index":
            quality.append(
                {
                    "severity": "error",
